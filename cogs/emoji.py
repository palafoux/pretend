...
class Emoji(commands.Cog):
   ...
   async def addemoji(self, ctx: commands.Context, emoji: Union[discord.Emoji, discord.PartialEmoji], *, name: str=None):
    ...
    except discord.HTTPException as e: return await ctx.send_error(ctx, f"Unable to add the emoji | {e}")
   ...
   async def addmultiple(self, ctx: commands.Context, *emoji: Union[discord.Emoji, discord.PartialEmoji]): 
    if len(emoji) == 0: return await ctx.send_warning("Please provide some emojis to add")       
    emojis = []
   ...
   async def sticker_delete(self, ctx: commands.Context): 
    if ctx.message.stickers: 
     ...
     if sticker.guild.id != ctx.guild.id: return await ctx.send_warning("This sticker is not from this server")
