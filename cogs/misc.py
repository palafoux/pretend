...
class Misc(commands.Cog):
  ...
  async def autotags_set(self, ctx: commands.Context, *, channel: discord.TextChannel):
   check = await self.bot.db.fetchrow("SELECT * FROM autotags WHERE guild_id = $1", ctx.guild.id) 
   async def create_idk(): 
    webhook = await channel.create_webhook(name="pretend - autolookup", avatar=await self.bot.user.display_avatar.read(), reason=f"autolookup channel configured by {ctx.author}")
    await self.bot.db.execute("INSERT INTO autotags VALUES ($1,$2)", ctx.guild.id, webhook.url) 
   if not check: await create_idk()
   else: 
    try:
     ...
     await self.bot.db.execute("DELETE FROM autotags WHERE guild_id = $1", ctx.guild.id)
     await create_idk()
   return await ctx.send_success(f"The **autolookup** channel is now configured to **{channel.mention}**")    
  ...
  async def autotags_unset(self, ctx: commands.Context): 
   check = await self.bot.db.fetchrow("SELECT * FROM autotags WHERE guild_id = $1", ctx.guild.id) 
   if not check: return await ctx.send_warning("**Autolookup** is not confugured")
   try:
     webhook = discord.Webhook.from_url(check['url'], session=self.bot.session)
     await webhook.delete(reason=f"autolookup channel removed by {ctx.author}")
   except: pass 
   await self.bot.db.execute("DELETE FROM autotags WHERE guild_id = $1", ctx.guild.id)
   return await ctx.send_success("Removed the **autolookup** channel") 
