...
def is_antinuke():
 async def predicate(ctx: commands.Context): 
  ...
  if not check: await ctx.send_warning("The antinuke is **not** enabled")
...
class AntiNuke(commands.Cog):
   ...
   async def sendlogs(self, action: str, member: discord.Member, punishment: str): 
    ...
    embed = discord.Embed(color=self.bot.color, title=f"{member} punished", description="Was pretend fast? If yes, please give us a review in https://discord.gg/pretend")
   ...
   @commands.hybrid_group(invoke_without_command=True, name="antinuke", description="protect your server against nukes", aliases=["an"])    
   ...
   @antinuke.group(name="admin", invoke_without_command=True, description="manage antinuke admins")
   ...
   async def an_settings(self, ctx: commands.Context): 
    settings_enabled = {"antibot": self.bot.no, "ban": self.bot.no, "kick": self.bot.no, "channel": self.bot.no} 
    ...
    embed = discord.Embed(color=self.bot.color, description='\n'.join([f'**{m}:** {settings_enabled.get(m)}' for m in ['antibot', 'ban', 'kick', 'channel']]))
    embed.set_author(name=f"antinuke settings for {ctx.guild.name}") 
    embed.set_thumbnail(url=ctx.guild.icon)
    await ctx.reply(embed=embed)   
   
   @antinuke.group(name="channel", invoke_without_command=True, description="prevent members from deleting the server's channels")
   async def an_channels(self, ctx): 
    return await ctx.create_pages()
   
   @an_channels.command(name="enable", aliases=['e'], description="enable channel delete protection", usage="[punishment]", brief="antinuke admin", help="antinuke")
   @Perms.check_whitelist("antinuke")
   @is_antinuke()
   async def an_channels_enable(self, ctx: commands.Context, *, punishment: str): 
    ...
    return await ctx.send_success(f"**Anti channel delete** enabled\npunishment: {punishment}\nThreshold: 1 channel deletion per 1 minute")
   
   @an_channels.command(name="disable", aliases=['dis'], description='disable channel delete protection', help="antinuke", brief="antinuke admin")
   @Perms.check_whitelist("antinuke")
   @is_antinuke()
   ...
   async def an_channels_threshold(self, ctx: commands.Context, number: int): 
    ...
    return await ctx.send_success(f"Antinuke channel delete threshold changed to **{number}** kicks per **60** seconds")

   @antinuke.group(name="kick", invoke_without_command=True, description="prevent members from mass kicking other members")
   ...
   async def an_kick_enable(self, ctx: commands.Context, *, punishment: str): 
    ...
    return await ctx.send_success(f"**Anti kick** enabled\npunishment: {punishment}\nThreshold: 1 kick per 1 minute")
   ...
   async def an_kick_threshold(self, ctx: commands.Context, number: int): 
    ...
    return await ctx.send_success(f"Antinuke kicks threshold changed to **{number}** kicks per **60** seconds")

   @antinuke.group(name="ban", invoke_without_command=True, description="prevent members from mass banning other members")
   ...
   async def an_ban_enable(self, ctx: commands.Context, *, punishment: str): 
    ...
    return await ctx.send_success(f"**Anti Ban** enabled\npunishment: {punishment}\nThreshold: 1 ban per 1 minute")
   ...
   async def an_ban_threshold(self, ctx: commands.Context, number: int): 
    ...
    return await ctx.send_success(f"Antinuke bans threshold changed to **{number}** bans per **60** seconds")

   @antinuke.group(name="botadd", aliases=['antibot'], invoke_without_command=True, description="prevent bots from joining your server")
